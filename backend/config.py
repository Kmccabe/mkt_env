...
class Settings(BaseSettings):
    ...
    # Production deployment port (Railway sets PORT environment variable)
    port: int = Field(
        default_factory=lambda: int(os.getenv("PORT", 8001)),
        ge=1000,
        le=65535,
        description="Server port (uses Railway's PORT in production)"
    )
    
    # Host setting for deployment
    host: str = Field(
        default="127.0.0.1",  # Local development
    ...
    cors_origins: List[str] = Field(
        default_factory=lambda: [
            ...
            "http://localhost:3000",
            "http://127.0.0.1:3000"
            # Students will add their deployed frontend URLs here
        ],
    ...
    def effective_host(self) -> str:
        """Get the effective host based on environment."""
        if self.is_production or os.getenv("PORT"):
            return "0.0.0.0"  # Listen on all interfaces in production
        return self.host  # Use configured host for development
    
    @property
    def effective_port(self) -> int:
        """Get the effective port, preferring Railway's PORT if available."""
        return self.port  # Already handles PORT environment variable
    
    def get_cors_origins(self) -> List[str]:
        """Get CORS origins based on environment."""
        if self.is_production:
            # In production, include all configured origins
